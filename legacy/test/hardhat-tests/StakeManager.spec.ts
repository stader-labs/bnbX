...
//   let relayFee: BigNumberish;
//   let feeBps: BigNumberish;
...
//       await signerBnbX.approve(dStakeManager.getAddress(), amount);
//     };
//   });
...
//     const bnbxFactory = await ethers.getContractFactory("BnbX");
//     bnbX = (await upgrades.deployProxy(bnbxFactory, [
//       admin.address,
//     ])) as unknown as BnbX;
//     bnbX = await bnbX.waitForDeployment();
...
//       [
//         await bnbX.getAddress(),
//         admin.address,
//         manager.address,
//         await tokenHub.getAddress(),
//         bcDepositWallet.address,
//         bot.address,
//         feeBps,
//       ]
//     )) as unknown as StakeManager;
//     await dStakeManager.waitForDeployment();

//     await bnbX.setStakeManager(dStakeManager.getAddress());
...
//       const amount = Number.from("300");
//       const zeroBalance = Number.from("0");
...
//       await uStakeManager.deposit({ value: ethers.parseEther("0.9") });
//       await expect(
//         bStakeManager.startDelegation({ value: relayFee })
//       ).to.be.revertedWith("Insufficient Deposit Amount");
//     });
...
//       await expect(
//         uStakeManager.requestWithdraw(ethers.parseEther("1"))
...
//       await bnbXApprove(user, amount.add(amount)); // Just to provide more allowance
//       await expect(
//         uStakeManager.requestWithdraw(ethers.parseEther("3"))
...
//       const amount = ethers.parseEther("2.1");
...
//       const amount = ethers.parseEther("2");
//       const dust = 56782;
//       const bnbXToWithdraw = ethers.parseEther("1").add(dust); ;
...
//       const amount = ethers.parseEther("1.234453748767838949");
//       await uStakeManager.deposit({ value: amount });
//       await bStakeManager.startDelegation({ value: relayFee });
//       await bStakeManager.completeDelegation(0);

//       await bStakeManager.addRestakingRewards(0, 8273873));
...
//       const depositAmount = ethers.parseEther("1.234453748767838949");
